#!/usr/bin/env python3
"""
T7: T6 + handedness-aware pointId features.

Per rule.md: pointId is in receiver's body frame (forehand/backhand). Players with
opposite handedness see MIRROR pointId distributions. Diagnostic confirms ~25% of
train players have inverted ratio_1_to_3 (likely left-handed receivers).

New features:
- srv_handedness_proxy = log P(rcv pointId=1) / P(rcv pointId=3) for server
  (when server is the RECEIVING side in other rallies — measures their own handedness)
- rcv_handedness_proxy = same for receiver
- is_mixed_handedness (binary cross)
- last_pointId_canonical: mirrored if receiver is inferred left-handed
- Trajectory features in canonical frame (consistent across handedness)

Usage:
    python3 src/train/predict_sgp_t7_handedness.py
"""
import os
import pathlib
...
# pointId mirror map: 1↔3, 4↔6, 7↔9 (forehand-side ↔ backhand-side); 2/5/8 unchanged
POINTID_MIRROR = {0:0, 1:3, 2:2, 3:1, 4:6, 5:5, 6:4, 7:9, 8:8, 9:7}


def compute_handedness_proxy(train_df, restrict_uids=None):
    """Per player, log ratio P(pointId=1 as receiver) / P(pointId=3 as receiver).
       Higher = forehand-short dominates → likely right-handed.
       Lower (negative) = backhand-short dominates → likely left-handed (mirror)."""
    if restrict_uids is not None:
        df = train_df[train_df["rally_uid"].isin(restrict_uids)]
    else:
        df = train_df
    # As RECEIVER: gamePlayerOtherId is the player whose frame the pointId is in
    counts = df.groupby(["gamePlayerOtherId", "pointId"]).size().unstack(fill_value=0)
    # Compute log(P(1)/P(3)) safely
    p1 = counts.get(1, pd.Series(0, index=counts.index)).astype(float)
    p3 = counts.get(3, pd.Series(0, index=counts.index)).astype(float)
    proxy = np.log((p1 + 1) / (p3 + 1))  # add-1 smoothing
    n_obs = (counts.sum(axis=1)).astype(int)
    out = {}
    for pid in counts.index:
        out[int(pid)] = {
            "proxy": float(proxy[pid]),
            "n_obs": int(n_obs[pid]),
            "is_lefty_inferred": int(proxy[pid] < 0),  # threshold at 0 (median around 1.2)
        }
    return out
...
def make_feats(rows, k, eb_srv, eb_rcv, srv_groups, rcv_groups, params_srv, params_rcv,
    ...
    # T2 trajectory (in original receiver-frame pointId)
    xs, ys = [], []
    for r in visible:
    ...
    # === T7 NEW: Handedness features ===
    # Server's handedness inferred from their RECEIVING patterns (when they were the gamePlayerOtherId)
    srv_h = handedness_proxy.get(srv_pid, {"proxy": 0.0, "n_obs": 0, "is_lefty_inferred": 0})
    rcv_h = handedness_proxy.get(rcv_pid, {"proxy": 0.0, "n_obs": 0, "is_lefty_inferred": 0})
    f["srv_handedness_proxy"] = srv_h["proxy"]
    f["srv_handedness_nobs"] = srv_h["n_obs"]
    f["srv_inferred_lefty"] = srv_h["is_lefty_inferred"]
    f["rcv_handedness_proxy"] = rcv_h["proxy"]
    f["rcv_handedness_nobs"] = rcv_h["n_obs"]
    f["rcv_inferred_lefty"] = rcv_h["is_lefty_inferred"]
    f["is_mixed_handedness"] = int(srv_h["is_lefty_inferred"] != rcv_h["is_lefty_inferred"])
    f["handedness_proxy_diff"] = srv_h["proxy"] - rcv_h["proxy"]

    # Canonical-frame pointId: if receiver is inferred lefty, mirror the pointId
    # The receiver of stroke i is the OPPOSITE side (gamePlayerOtherId for stroke 1 = receiver of stroke 1)
    # For each visible stroke, who is the receiver?
    # Stroke i (strikeNumber i): hit by hitter (gamePlayerId for odd-i = server, for even-i = receiver).
    # Ball lands on the OTHER side. For odd strike (server hits), ball lands on receiver side
    # = gamePlayerOtherId of rally. For even strike, ball lands on server side = gamePlayerId.
    # → We need each ball's LANDER's handedness:
    #   odd strike: lander = gamePlayerOtherId = rcv_pid → use rcv_h
    #   even strike: lander = gamePlayerId = srv_pid → use srv_h
    last_strike_num = last["strikeNumber"]
    if last_strike_num % 2 == 1:
        # Server hit, ball lands on receiver's side (rcv_pid's frame)
        last_lander_lefty = rcv_h["is_lefty_inferred"]
    else:
        last_lander_lefty = srv_h["is_lefty_inferred"]
    f["last_pointId_canonical"] = (
        POINTID_MIRROR.get(last["pointId"], last["pointId"])
        if last_lander_lefty else last["pointId"]
    )
    f["last_point_side_canonical"] = POINT_SIDE.get(f["last_pointId_canonical"], 0)
    if k >= 2:
        prev_strike_num = visible[-2]["strikeNumber"]
        if prev_strike_num % 2 == 1:
            prev_lander_lefty = rcv_h["is_lefty_inferred"]
        else:
            prev_lander_lefty = srv_h["is_lefty_inferred"]
        prev_pid = visible[-2]["pointId"]
        f["prev_pointId_canonical"] = (
            POINTID_MIRROR.get(prev_pid, prev_pid) if prev_lander_lefty else prev_pid
        )
    else:
        f["prev_pointId_canonical"] = -1
...
def main():
    print("=" * 72)
    print("T7: T6 + handedness-aware features")
    print("=" * 72)
    ...
    for fold, (tr_idx, va_idx) in enumerate(sgkf.split(rally_meta, uid_arr, groups=match_arr)):
        ...
        # T7: Compute handedness proxy from training fold (combine train + test for full inference of test players)
        # For training-fold inference: only use training-fold rallies
        # For test players (test rallies): can use test rallies too since they don't leak sgp labels
        handedness_proxy_train = compute_handedness_proxy(train_df, restrict_uids=tr_uids)
        # Add test players' handedness (computed from test rally patterns; doesn't leak sgp)
        handedness_proxy_test = compute_handedness_proxy(test_df, restrict_uids=set(test_df["rally_uid"].unique()))
        handedness_proxy = {**handedness_proxy_train, **handedness_proxy_test}  # test overrides if both exist; but disjoint usually
        ...
        X_tr, uid_tr, R_tr, sgp_tr, k_tr = build_features(tr_rdf, eb_srv, eb_rcv, srv_groups, rcv_groups,
                                                          ...
                                                          eb_srv, eb_rcv, handedness_proxy,
        ...
        new_t7 = [c for c in feat_cols_ref if c.startswith(("srv_handedness", "rcv_handedness",
                                                              "is_mixed_handedness", "handedness_proxy_diff",
                                                              "srv_inferred_lefty", "rcv_inferred_lefty",
                                                              "last_pointId_canonical", "prev_pointId_canonical",
                                                              "last_point_side_canonical"))]
        if fold == 0:
            print(f"\nFold 1: total feats={X_tr.shape[1]}, T7 new={len(new_t7)}: {new_t7}")
            n_lefty = sum(1 for v in handedness_proxy.values() if v["is_lefty_inferred"])
            print(f"  Inferred lefties: {n_lefty} of {len(handedness_proxy)} ({100*n_lefty/max(len(handedness_proxy),1):.1f}%)")
        ...
        new_in_top10 = sum(1 for n, _ in top10 if n in new_t7)
        print(f"  Fold {fold+1} prefix-AUC: {prefix_auc:.4f}, best_iter: {model.best_iteration}, T7-new-in-top10: {new_in_top10}/10")
    ...
    print(f"\n{'='*72}")
    print(f"T7 OOF AUC (per-rally aggregated): {overall_auc:.4f}")
    ...
    for model, (eb_srv, eb_rcv, srv_groups, rcv_groups, params_srv, params_rcv,
                ...
                per_player_logp, per_player_n, global_logp,
                player_nmf, train_player_pids, train_V_norm,
                handedness_proxy) in zip(fold_models, fold_artifacts):
    ...
    out_path = "artifacts/sgp_pred_NEWTEST_t7_handedness.npz"
    np.savez(out_path, rally_uids=test_uids_ref, sgp_pred=test_pred,
             method="t7_lgb_R_eb_markov_nmf_handedness",
